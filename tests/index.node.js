...
var nettype = worktips.nettype_utils.network_type.MAINNET;
...
{
	try {
		var decoded = (await worktips.worktips_utils_promise).decode_address(
	...
	try {
		var created = (await worktips.worktips_utils_promise).newly_created_wallet(
			"ja",
			nettype,
		);
		...
		//
		try {
			var unpacked = (await worktips.worktips_utils_promise).seed_and_keys_from_mnemonic(
	...
	try {
		var fee = new worktips.JSBigInt((await worktips.worktips_utils_promise).estimated_tx_network_fee(
			"0", 1, "24658"
			// fee_per_kb__string, priority, fee_per_b__string
		));
		console.log("estimated_tx_network_fee", worktips.worktips_amount_format_utils.formatMoneyFull(fee), "XMR")
	} catch (e) {
		console.log(e)
	}
	...
	try {
		...
		const reason = worktips.worktips_txParsing_utils.TransactionLockedReason(tx, blockchain_height)
	...
	try {
		const blockchain_height = worktips.worktips_config.maxBlockNumber
		const tx = 
		{
